...
def validate_rows(rows: list[dict[str, Any]], *, training_seeds: tuple[int, ...]) -> None:
    expected = {
...
def paired_effects(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for scenario in SCENARIOS:
        for baseline in EXTERNAL_METHODS:
            for metric in PRIMARY_METRICS:
                ...
                if not differences:
                    raise ValueError(f"empty paired contrast for {scenario}/{baseline}/{metric}")
    ...
    if any(row["baseline"] == PROPOSED_METHOD for row in output):
        raise AssertionError("proposed method must not appear as its own paired baseline")
    return output
...
def write_external_comparison_artifacts(output_dir: Path, *, episode_rows: list[dict[str, Any]],
    ...
    validate_rows(summary_rows, training_seeds=seeds)
    ...
    effects = paired_effects(summary_rows)
    ...
    (output_dir / "tables" / "external_comparison.md").write_text(_table(statistics), encoding="utf-8")
