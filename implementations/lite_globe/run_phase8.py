"""Optimize Lite-GLOBE with geographic residual distillation."""
...
from .evaluation import write_phase8_artifacts
from .experiments import Phase8Config, run_phase8_campaign
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=package / "config" / "phase8.yaml",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/lite_globe/phase8"),
    )
...
def _config(raw: dict, smoke: bool) -> Phase8Config:
    ...
    if smoke:
        return Phase8Config(
            ...
            calibration_episodes_per_stage=8,
            structural_hole_episodes_per_variant=6,
            calibration_pdr_tolerance=float(
                training["calibration_pdr_tolerance"]
            ),
            prior_pretraining_epochs=8,
        )
    return Phase8Config(
        ...
        prior_pretraining_epochs=int(
            training["prior_pretraining_epochs"]
        ),
    )
