"""Run Phase 9 risk-aware optimization, ablations, and stress tests."""
...
from .evaluation import write_phase9_artifacts
from .experiments import Phase9Config, run_phase9_campaign
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=package / "config" / "phase9.yaml",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/lite_globe/phase9"),
    )
...
def _config(raw: dict, smoke: bool) -> Phase9Config:
    ...
    if smoke:
        return Phase9Config(
            ...
            initial_risk_strength=tuple(
                float(value)
                for value in training["initial_risk_strength"]
            ),
    ...
    return Phase9Config(
        ...
        risk_pretraining_epochs=int(
            training["risk_pretraining_epochs"]
        ),
    )
