...
최종 출력 레이어는 현재 패킷 포워딩을 수행하려는 노드의 1-hop 유효 이웃들과 드롭 행동만을 고르는 구조적 마스킹(Action Masking)을 반영한 카테고리컬 액터(Masked Categorical Actor)를 사용하여 포워딩 행동 분포 $\pi_T(\cdot \mid s_t)$를 결정한다. 학습은 완전 에피소드 리턴 및 클리핑 기반 PPO(Proximal Policy Optimization)\cite{schulman2017ppo}와 집중형 가치 네트워크(Centralized Value Network)를 연계해 CPU/CUDA 환경에서 오프라인으로 수렴될 때까지 수행된다.
...
\begin{algorithm}
\caption{\method{} 분산형 패킷 포워딩 의사결정 알고리즘}
\label{alg:risk_switch_kci}
\begin{algorithmic}[1]
\Require 현재 노드 $e_i$, 수신 패킷 $p$, 이웃 집합 $\mathcal{N}(i)$, 임계값 $\eta$
\Ensure 차홉 전달 노드 $a_i \in \mathcal{N}(i) \cup \{\dropact\}$
\State 현재 큐 길이 $q_i$ 및 이웃의 잔여 수명 $L_j$ 수집
\State 무선 패킷 손실률 $p_{i,\mathrm{loss}}$ 연산
\State Danger Score 연산: $DS_i \leftarrow w_Q \frac{q_i}{q_{\mathrm{max}}} + w_L \sigma_L(L_{\mathrm{thr}} - L_i) + w_E p_{i,\mathrm{loss}}$
\If{$DS_i \le \eta$} \Comment{정상 모드: 지리적 라우팅 수행}
    \State $a_i \leftarrow \mathrm{argmax}_{j \in \mathcal{N}(i)} \mathrm{GeographicProgress}(j, e_d)$
\Else \Comment{위험 모드: 예측 신경망 활성화}
    \State 로컬 관측 $o_{i,t}$ 획득 및 인코딩 $\mathbf{z}_{i,t}$
    \State 확률 분포 도출: $\mathbf{P} \leftarrow \pi_S(\cdot \mid o_{i,t})$
    \State $a_i \leftarrow \mathrm{sample}(a \sim \mathbf{P})$
    \If{$a_i = \dropact$ {\bf and} 대안 노드 존재} \Comment{드롭 억제}
        \State $a_i \leftarrow \mathrm{argmax}_{j \in \mathcal{N}(i)} \mathrm{SafetyUtility}(j)$ \Comment{Tie-breaking}
    \EndIf
\EndIf
\State \Return $a_i$
\end{algorithmic}
