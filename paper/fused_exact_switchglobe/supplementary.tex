...
\firstpage{1}\makeatletter\setcounter{page}{\@firstpage}\makeatother
...
\Title{Supplementary Material for ``Fused Exact SwitchGLOBE''}
\Author{Anonymous Author(s)}\AuthorNames{Anonymous Author(s)}
\address{Affiliation withheld for double-blind draft preparation}
\corres{Correspondence: to be completed before submission}
\abstract{This supplement documents experimental provenance, metric semantics, acceptance gates, negative results, and the complete 20-figure latency and reliability analysis supporting the Fused Exact SwitchGLOBE manuscript.}
\keyword{supplementary material; UAV routing; inference latency; reproducibility}
\begin{document}

\section{Artifact and Provenance Inventory}

Table~\ref{tab:inventory} maps each claim family to its authoritative local artifact. Paths are relative to the project root. The paper copies publication-facing tables and figure sources into \texttt{paper/fused\_exact\_switchglobe}; the immutable experiment folders remain the source of record.
...
Exact--Fast reliability & R2 & Paired seed differences; predeclared 0.5 percentage-point non-inferiority floor \\
...
\caption{Mean, median, p95, and p99 latency summaries for exact and candidate variants. Fast variants are approximate policies and require a separate reliability judgment.}
...
\caption{Paired p95 speed effects. Positive is faster than fused Exact. Confidence intervals are computed across matched checkpoints.}
...
\caption{Deadline-delivery ratio versus A100 p95 decision latency. Exact occupies the reliability-oriented point; Fast occupies a lower-latency but lower-reliability point.}
\label{fig:s06}\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{supplementary_figures/07_reliability_heatmap.png}
\caption{Scenario-level reliability heatmap. It reveals that Fast degradation is concentrated in specific OOD node-density and predictive-break conditions rather than uniform across the benchmark.}
...
\caption{Switch activation by scenario. A switched action and the need to compute the predictive branch are different events: Exact computes both branches, while Early Exit attempts to skip a branch before the switch can be fully evaluated.}
...
\caption{Parameter and serialized-size comparison. Early Exit shares the Exact weights; Fast contains 7,011 parameters and is a separately distilled approximate model.}
...
\caption{Candidate decision matrix. Fused Exact improves the duplicated legacy execution without an outcome change; buffering is too small; Early Exit is slower; Fast violates the quality gate.}
...
\midrule
Fused Exact & CPU & 2.168 & 2.161 & 2.208 & 2.344 & retain \\
Early Exit & CPU & 2.366 & 2.356 & 2.427 & 2.581 & reject \\
Fast & CPU & 0.812 & 0.814 & 0.847 & 0.927 & quality fail \\
Fast + Top-2 & CPU & 0.853 & 0.852 & 0.881 & 0.933 & quality unresolved \\
Buffered Exact & CPU & 2.187 & 2.179 & 2.244 & 2.370 & hold \\
Legacy repeated & CPU & 3.698 & 3.687 & 3.775 & 3.935 & superseded \\
Fused Exact & CUDA & 5.671 & 5.648 & 5.836 & 6.101 & retain \\
Early Exit & CUDA & 6.198 & 6.172 & 6.386 & 6.663 & reject \\
Fast & CUDA & 1.957 & 1.950 & 2.005 & 2.120 & quality fail \\
Fast + Top-2 & CUDA & 2.040 & 2.032 & 2.092 & 2.183 & quality unresolved \\
Buffered Exact & CUDA & 5.626 & 5.602 & 5.788 & 6.020 & hold \\
...
\begin{table}[H]
\caption{Paired CUDA p95 effects relative to Fused Exact. Positive reduction means faster.}\label{tab:paired}\small
\begin{tabularx}{\textwidth}{lYYYY}
\toprule Candidate & p95 reduction & 95\% $t$ CI & Bootstrap 95\% CI & Interpretation \\
\midrule
Early Exit & $-9.42\%$ & [$-10.62$,$-8.22$] & [$-10.22$,$-8.61$] & slower \\
Buffered Exact & $+0.83\%$ & [$+0.21$,$+1.44$] & [$+0.48$,$+1.25$] & too small; CPU reverses \\
Fast & $+65.65\%$ & [$+65.06$,$+66.24$] & [$+65.24$,$+65.95$] & latency pass, quality fail \\
Fast + Top-2 & $+64.16\%$ & [$+63.65$,$+64.66$] & [$+63.84$,$+64.50$] & latency pass, quality unresolved \\
\bottomrule
\end{tabularx}
\end{table}

\section{Exact--Fast Reliability Gate}

\begin{table}[H]
\caption{Fast minus Exact directional paired effects. Higher-is-better metrics use Fast$-$Exact; lower-is-better metrics use Exact$-$Fast, so negative always means degradation.}\label{tab:fast}\small
\begin{tabularx}{\textwidth}{lYYYYY}
\toprule Metric & Exact & Fast & Directional change & 95\% CI & Gate \\
\midrule
...
The connected-PDR interval lies entirely below zero and its lower bound is far below the allowed $-0.005$. Fast therefore cannot be called statistically equivalent or non-inferior to Exact. The deadline interval overlaps zero at its upper edge, but its lower edge independently violates the gate. The delay and energy directions reinforce the rejection.

\section{Operator-Profile Interpretation}

The Exact profile contains 1,050 \texttt{aten::\_local\_scalar\_dense} events and the same number of device-to-host copies over 30 calls. Early Exit contains 1,140 of each; Fast contains 300; Fast+Top-2 contains 330. The counts are evidence of synchronization frequency, not time percentages for the primary benchmark. Exact also shows Boolean reductions such as \texttt{aten::all} and \texttt{aten::any}, which are consistent with action-mask and gate logic.

This evidence motivates three implementation hypotheses: (i) preserve gate and action extraction as tensors until the final API boundary; (ii) replace Python branching with a runtime-native conditional graph; and (iii) fuse small elementwise and reduction operations where the deployment compiler can do so without altering masks or threshold comparisons. Any optimized graph must pass exact logit/action replay or the same routing-quality gate used for Fast.
...
\item Verify SHA-256 for the source bundle and five Exact/Fast checkpoint archives.
...
Before submission, the authors must replace anonymous metadata, verify the current Drones template version, insert a public repository/DOI or a defensible restricted-access statement, add the precise simulator license, state the compute resource provider, and decide whether adapted baseline code can be released. A target-device experiment is strongly recommended. If it cannot be completed, the title, abstract, and conclusion must retain the term ``deployment-oriented'' rather than ``deployed'' or ``real-time onboard.''
