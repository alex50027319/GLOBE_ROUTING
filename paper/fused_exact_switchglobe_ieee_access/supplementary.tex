...
\vol{14}
\year{2026}
\history{Supplementary material prepared September 5, 2026.}
\doi{10.1109/ACCESS.2026.0000000}
\title{Supplementary Material for ``Fused Exact SwitchGLOBE''}
\author{\uppercase{Anonymous Author(s)}\authorrefmark{1}}
\address[1]{Affiliation withheld for double-blind draft preparation}
\tfootnote{Funding information will be completed before submission.}
\markboth{Anonymous Author(s): Supplementary Material for Fused Exact SwitchGLOBE}{Anonymous Author(s): Supplementary Material for Fused Exact SwitchGLOBE}
\corresp{Corresponding author: to be completed before submission.}
\begin{abstract}
This supplementary document records experimental provenance, metric semantics, statistical units, reliability acceptance gates, negative results, and the complete 20-figure latency and reliability portfolio supporting the Fused Exact SwitchGLOBE article. It also provides the exact-versus-Fast gate table, operator-profile interpretation, and a reproduction and submission checklist.
\end{abstract}
\begin{keywords}
...
Table~\ref{tab:inventory} maps each claim family to its authoritative local artifact. Paths are relative to the project root. This IEEE Access package copies publication-facing tables and figure sources into \texttt{\seqsplit{paper/fused\_exact\_switchglobe\_ieee\_access}}; the immutable experiment folders remain the source of record.
...
Before submission, the authors must replace anonymous metadata, verify the current IEEE Access template version, insert a public repository/DOI or a defensible restricted-access statement, add the precise simulator license, state the compute resource provider, and decide whether adapted baseline code can be released. A target-device experiment is strongly recommended. If it cannot be completed, the title, abstract, and conclusion must retain the term ``deployment-oriented'' rather than ``deployed'' or ``real-time onboard.''
