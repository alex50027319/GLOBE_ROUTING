...
\begin{algorithm}[t]
\caption{\method\ Decentralized Next-Hop Forwarding Decision}
\label{alg:risk_switch}
\begin{algorithmic}[1]
\Require Current node $u$, packet $p$ destined for $d$, neighbor set $\mathcal{N}_u(t)$, danger threshold $\tau$, safety-gain threshold $\delta$
\Ensure Forwarding action $a^\star \in \mathcal{N}_u(t) \cup \{\dropact\}$
\State Compute local observation $o_u(t)$ from self, neighbor, and edge features
\State $a_N \sim \pi_S(\cdot \mid o_u(t))$ \Comment{nominal distilled action}
\State Compute Danger Score $D_i$ and Safety Utility $Q_i$ for each $i \in \mathcal{N}_u(t)$ using risk features $x_i^{\mathrm{risk}}$ and P+ features $x_i^{+}$
\State $a_P \gets \arg\max_{i \in \mathcal{N}_u(t)} Q_i$ \Comment{predictive safety-branch candidate}
\State $G(a_P, a_N) \gets Q_{a_P} - Q_{a_N}$
\State $S \gets \mathbb{1}[\, a_N = \dropact \ \lor\ D_{a_N} > \tau \ \lor\ (a_P \neq a_N \land G(a_P,a_N) > \delta \land D_{a_P} < D_{a_N}) \,]$
\If{$S = 1$} \Comment{risk switch activated}
    \State $a^\star \gets a_P$
\Else
    \State $a^\star \gets a_N$
\EndIf
\If{$a^\star = \dropact$ \textbf{and} $\exists\, i \in \mathcal{N}_u(t)$ passing the basic safety gate} \Comment{drop suppression}
    \State $a^\star \gets \arg\max_{i \in \mathcal{N}_u(t)} \left(z_i^{P} + \lambda_E \eta_i\right)$ \Comment{energy-aware tie break}
\EndIf
\State \Return $a^\star$
\end{algorithmic}
\end{algorithm}
