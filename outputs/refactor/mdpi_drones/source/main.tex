...
% Submission-ready research draft. Author/funding/repository placeholders must
% be completed by the authors before submission.
...
\let\OriginalAbstractCommand\abstract
\renewcommand{\abstract}[1]{\OriginalAbstractCommand{{\color{ReviewBlue}#1}}}

\Title{\VerifyEdit{Fused Exact SwitchGLOBE: Exact-Semantics Global-to-Local Policy Distillation with Risk-Switched Predictive Recovery for Decentralized UAV Routing}}

\Author{\MustEdit{Kyung Eun Kim and Howon Lee}}
\AuthorNames{\MustEdit{Kyung Eun Kim and Howon Lee}}
\address{\MustEdit{Department of Military Digital Convergence, Ajou University, Suwon 16499, Republic of Korea}}
\corres{\MustEdit{Correspondence: Howon Lee; insert the final institutional e-mail address before submission}}

\abstract{Flying ad hoc networks require packet-forwarding decisions under rapid topology change, intermittent connectivity, and tight onboard compute budgets. Global graph policies can learn strong routing preferences, but their information requirements are incompatible with strictly decentralized forwarding; conversely, compact approximations may reduce inference time while silently degrading delivery reliability. We present \FSG{}, a deployment-oriented framework that combines three ideas. First, a graph-aware proximal-policy-optimization teacher is used only during training and its masked preferences are distilled into one-hop student policies. Second, a calibrated local risk switch selects between a geographic-residual normal branch and a predictive recovery branch using link margin, estimated lifetime, and onward-connectivity features. Third, exact-semantics execution fusion evaluates each branch once and derives the action and diagnostics from the same intermediate tensors, eliminating the legacy repeated-forward path without changing the policy definition. A common-simulator evaluation comprises eight methods, five training seeds, 14 held-out or stress scenarios, and 200 episodes per seed--scenario pair (112,000 episode records). \SG{} Exact attains a connected-pair packet delivery ratio of 0.9053 and a deadline-delivery ratio of 0.8376. Relative to the strongest adapted baseline, Evo-QGeo, the paired improvements are 1.84 percentage points (95\% CI: 1.52--2.16) and 2.23 points (1.89--2.57), respectively. In a same-session batch-one benchmark on an NVIDIA A100-SXM4-40GB, fusion reduces CUDA p95 decision latency from 10.004 to 5.836 ms (41.66\%) relative to the legacy repeated path while preserving exact behavior. The host CPU is faster at 2.208 ms p95, showing that GPU acceleration is not automatic for fine-grained online routing. A single-pass distilled variant reaches 2.005 ms on CUDA, but loses 1.81 PDR points (95\% CI: $-3.61$ to $-0.01$) and therefore fails a predeclared reliability gate. These results support an evidence hierarchy in which routing quality, decision latency, and runtime scope are measured separately and speedups are accepted only when forwarding behavior remains defensible.}

\keyword{\VerifyEdit{UAV networks; FANET; packet routing; reinforcement learning; policy distillation; graph neural network; conditional computation; inference latency; decentralized systems; reproducible simulation}}

\begin{document}

\noindent\fcolorbox{ReviewBlue}{white}{\parbox{0.96\textwidth}{\small\textbf{Editing-guide copy (not for submission).} \MustEdit{Red text requires author confirmation or replacement.} \VerifyEdit{Blue text must be revalidated whenever experiments, claims, figures, or positioning change.} Remove all review-color commands before submission.}}
...
\begin{align}
\max_{\pi} \quad & \E[\pdr_C(\pi)] + \lambda_D\E[\mathrm{DDR}(\pi)] - \lambda_L\E[L_{95}(\pi)]-\lambda_E\E[E_{\mathrm{proxy}}(\pi)],\\
\text{s.t.}\quad & a_t\in\mathcal{A}_u(t),\quad \pi(a_t|o_u(t)),\quad T_{\mathrm{dec},95}\leq B_T.
\end{align}
...
\begin{equation}
p_T^\tau(a)=\operatorname{softmax}\!\left(\frac{z_T(a)+\log M(a)}{\tau}\right),\quad
...
\begin{equation}
S=\mathbb{1}\left[a_N=\drop\;\lor\;D(a_N)>\eta\;\lor\;\left(a_P\neq a_N\land G(a_P,a_N)>0.10\land D(a_P)<D(a_N)\right)\right].
\label{eq:switch}
\end{equation}
...
\begin{equation}
\pdr_C=\frac{\sum_{e\in\mathcal E}Y_eC_e}{\sum_{e\in\mathcal E}C_e},
\end{equation}
and overall PDR is $|\mathcal E|^{-1}\sum_eY_e$. Deadline delivery ratio is $|\mathcal E|^{-1}\sum_eD_e$. The p95 successful delay is the 0.95 quantile of forwarding steps among delivered packets only. A missing-delivery cell is reported as N/A, never as zero; otherwise a policy that drops every packet could appear to have zero delay. Network end-to-end delay in steps and local decision latency in milliseconds are deliberately reported as different metrics.
...
\VerifyEdit{Table~\ref{tab:latency} reports the same-session A100 results. The fused Exact CUDA p95 is 5.836 ms, compared with 10.004 ms for legacy repeated execution: a 41.66\% reduction. On the Colab host CPU, the corresponding values are 2.208 and 3.775 ms, a 41.51\% reduction.} The exact policy result is unchanged because the speedup removes duplicate evaluation rather than changing a branch.
...
\begin{equation}
T_{\mathrm{E2E}}=\sum_{h=1}^{H}\left(T_{\mathrm{queue},h}+T_{\mathrm{MAC},h}+T_{\mathrm{tx},h}+T_{\mathrm{prop},h}+T_{\mathrm{decision},h}+T_{\mathrm{update},h}\right).
\end{equation}
