"""Generate the 20-figure DiSwitch latency and reliability evidence pack."""
...
COLORS = {
    "DiSwitch": "#173F5F", "Early Exit": "#F6AE2D", "Fast-DiSwitch": "#2A9D8F",
    "Fast-DiSwitch + Top-2": "#6A4C93", "Legacy": "#9B2226", "Buffered DiSwitch": "#577590",
}
...
def label(variant: str) -> str:
    return {
        "exact_eager_cpu": "DiSwitch", "early_exit_cpu": "Early Exit",
        "fast_eager_cpu": "Fast-DiSwitch", "fast_top2_eager_cpu": "Fast-DiSwitch + Top-2",
        "legacy_repeated_cpu": "Legacy", "exact_buffered_cpu": "Buffered DiSwitch",
    }.get(variant, variant)
...
def main() -> int:
    ...
    figures = args.output_dir / "supplementary_figures"; metrics = args.output_dir / "figure_data"
    ...
    captions.append(("02_latency_summary","동일 checkpoint와 5 seeds에서 mean/P50/P95/P99를 비교한다. Fast-DiSwitch 계열은 낮지만 reliability gate는 별도 판단한다."))
    ...
    ax.errorbar(m,y,xerr=[m-lo,hi-m],fmt="o",capsize=5,color="#173F5F"); ax.axvline(0,color="black",lw=1); ax.set_yticks(y,[r["variant"] for r in data]); ax.set_xlabel("Paired p95 reduction vs DiSwitch (%)")
    style(ax,"04  Paired seed-level p95 reduction (95% t-CI)")
    save(fig,figures,"04_latency_forest"); write(metrics/"figure_04.csv",data)
    captions.append(("04_latency_forest","양수는 DiSwitch 대비 빠름. Early Exit과 buffering은 CI가 0을 가로질러 개선으로 판정하지 않는다."))
    ...
    for num,metric,title in [(5,"connected_pair_pdr","Connected-pair PDR"),(6,"deadline_delivery_ratio","Deadline delivery ratio")]:
        row=q[metric]; pts=[("DiSwitch",exact_a100,float(row["exact_mean"])),("Fast-DiSwitch",fast_a100,float(row["fast_mean"]))]
    ...
    display_methods = ["DiSwitch", "Fast-DiSwitch"]
    ax.set_xticks(range(2),display_methods); ax.set_yticks(range(len(scenarios)),scenarios,fontsize=8); style(ax,"07  Scenario × method reliability heatmap")
    save(fig,figures,"07_reliability_heatmap"); write(metrics/"figure_07.csv",[{"scenario":s,"method":display_methods[j],"connected_pair_pdr":mat[i,j]} for i,s in enumerate(scenarios) for j in range(2)])
    ...
    ax.set_xticks(range(2),["DiSwitch replay","Early Exit"]); ax.set_yticks(range(len(scenarios)),scenarios,fontsize=8); style(ax,"08  Scenario latency diagnostic (separate runs)")
    save(fig,figures,"08_latency_heatmap"); write(metrics/"figure_08.csv",[{"scenario":s,"method":["DiSwitch","Early Exit"][j],"mean_episode_p95_ms":mat[i,j]} for i,s in enumerate(scenarios) for j in range(2)])
    ...
    fig,ax=plt.subplots(figsize=(10,5.5)); ax.bar(range(len(scenarios)),vals,color=COLORS["DiSwitch"]); ax.set_xticks(range(len(scenarios)),scenarios,rotation=55,ha="right",fontsize=8); style(ax,"10  DiSwitch activation by scenario","Activation rate")
    ...
    names=["DiSwitch","Early Exit","Fast-DiSwitch","Fast-DiSwitch + Top-2"]; vs=variants
    ...
    save(fig,figures,"14_model_footprint"); write(metrics/"figure_14.csv",[{"method":n,"parameters":p,"checkpoint_kib":s} for n,p,s in zip(names,params,sizes)]); captions.append(("14_model_footprint","Early Exit은 DiSwitch 가중치를 그대로 사용해 크기가 같고 Fast-DiSwitch는 7,011 parameters다."))
    ...
    for dev in ["cpu","cuda"]:
        for vn,n in [("SwitchGLOBE Exact","DiSwitch"),("FastSwitchGLOBE","Fast-DiSwitch")]:
    ...
    for dev in ["cpu","cuda"]:
        for vn,n in [(f"exact_eager_{dev}","DiSwitch"),(f"fast_eager_{dev}","Fast-DiSwitch")]:
    ...
    for j,n in enumerate(["DiSwitch","Fast-DiSwitch"]): ax.bar(x+(j-.5)*.36,[next(r["p95_ms"] for r in points if r["environment"]==e and r["method"]==n) for e in envs],.36,label=n,color=COLORS[n])
    ...
    fig,ax=plt.subplots(figsize=(8.5,5.5)); ax.bar([str(r["seed"]) for r in data],[r["early_minus_exact_ms"] for r in data],color=["#2A9D8F" if r["early_minus_exact_ms"]<0 else "#9B2226" for r in data]); ax.axhline(0,color="black",lw=1); style(ax,"19  Early Exit paired p95 difference","Early Exit − DiSwitch (ms; lower better)")
    ...
    candidates=[
        ("DiSwitch",100*(a100_legacy-a100_exact)/a100_legacy,0.0,"adopt"),
        ("Buffered DiSwitch",100*(a100_exact-a100_buffered)/a100_exact,0.0,"hold"),
        ("torch.compile (old)",100*(oldexact-compiled)/oldexact,0.0,"reject"),
        ("Early Exit",100*(a100_exact-a100_early)/a100_exact,0.0,"reject"),
        ("Fast-DiSwitch",100*(a100_exact-fast_a100)/a100_exact,100*float(q["connected_pair_pdr"]["directional_difference_mean"]),"trade-off"),
        ("Fast-DiSwitch + Top-2",100*(a100_exact-a100_fast_top2)/a100_exact,100*float(q["connected_pair_pdr"]["directional_difference_mean"]),"trade-off"),
    ]
    fig,ax=plt.subplots(figsize=(9,5.8)); cmap={"adopt":"#2A9D8F","reject":"#9B2226","hold":"#F6AE2D","trade-off":"#6A4C93"}
    offsets={"DiSwitch":(-50,-25),"Buffered DiSwitch":(8,-27),"torch.compile (old)":(-12,-30),"Early Exit":(-30,16),"Fast-DiSwitch":(-20,14),"Fast-DiSwitch + Top-2":(-15,-28)}
    ...
    save(fig,figures,"20_failed_candidate_tradeoff"); write(metrics/"figure_20.csv",[{"candidate":n,"p95_reduction_percent":x,"connected_pdr_change_pp":y,"decision":s} for n,x,y,s in candidates]); captions.append(("20_failed_candidate_tradeoff","오른쪽 위가 바람직하다. DiSwitch는 동작 동일성을 보존하면서 legacy 중복-forward 대비 2026-09-05 A100에서 개선됐다. Fast-DiSwitch는 reliability gate를 통과하지 못했다."))
