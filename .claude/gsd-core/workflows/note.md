...
- **Global scope**: `/home/alex/GLOBE_ROUTING/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/alex/GLOBE_ROUTING/.claude/notes/`)
...
2. Glob `/home/alex/GLOBE_ROUTING/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/alex/GLOBE_ROUTING/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/alex/GLOBE_ROUTING/.claude/notes/` — works in any directory
