"""Paper-ready reports for Phase 13 Risk-Switch Lite-GLOBE-P+."""
...
from ..experiments.phase13_campaign import PHASE13_METHODS
...
def aggregate_phase13(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def paired_phase13_effects(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_key = {
        (
            str(row["scenario"]),
            str(row["method"]),
...
def _lookup(rows: list[dict[str, Any]]) -> dict[tuple[str, str, str], dict[str, Any]]:
    return {
...
def _main_table(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "# Phase 13 Risk-Switch Lite-GLOBE-P+ Results",
        "",
        "| Method | Scenario | Connected PDR | Deadline delivery | "
        "Delay p95 | Energy proxy | Input bytes | Switch steps |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for scenario in PHASE13_SCENARIOS:
        for method in PHASE13_METHODS:
...
def _plot_metric(
    ...
    *,
    ...
    for method in PHASE13_METHODS:
        ...
        axis.plot(PHASE13_SCENARIOS, means, marker="o", label=method)
    if "pdr" in metric or "ratio" in metric:
        axis.set_ylim(-0.02, 1.02)
    axis.set_ylabel(ylabel)
    axis.set_title(f"Phase 13 {ylabel}")
...
def write_phase13_artifacts(
    ...
    *,
    ...
    aggregates = aggregate_phase13(summary_rows)
    effects = paired_phase13_effects(summary_rows)
    ...
    (tables / "risk_switch_plus_results.md").write_text(
        _main_table(aggregates), encoding="utf-8"
    )
    (tables / "risk_switch_plus_paired_effects.md").write_text(
    ...
    _plot_metric(
        ...
        path=figures / "risk_switch_plus_pdr",
    )
    _plot_metric(
        aggregates,
        metric="p95_success_delay",
        ylabel="P95 Delay",
        path=figures / "risk_switch_plus_delay_p95",
    )
    _plot_metric(
        aggregates,
        metric="mean_policy_input_bytes",
        ylabel="Policy Input Bytes",
        path=figures / "risk_switch_plus_input_bytes",
    )
    _plot_metric(
        aggregates,
        metric="mean_switch_steps",
        ylabel="Mean Switch Steps",
        path=figures / "risk_switch_plus_switch_steps",
    )
    manifest = {
        **metadata,
        ...
        "scenarios": list(PHASE13_SCENARIOS),
        "primary_metric": "connected_pair_pdr",
        "proposed_method": PROPOSED_METHOD,
        "metric_policy": (
            "PDR and deadline delivery are primary; delay, energy, bytes, "
            "agent drops, and switch steps are secondary constraints."
        ),
        "artifacts": sorted(
