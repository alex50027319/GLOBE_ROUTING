"""Paper-ready reports for Phase 10 external RL baselines."""
...
from ..experiments.phase10_campaign import PHASE10_METHODS
...
PHASE10_SCENARIOS = tuple(
    scenario.name for scenario in phase9_evaluation_scenarios(0)
)
PROPOSED_METHOD = "Phase 8 Geo-Residual KD"
EXTERNAL_BASELINES = (
    "GPSR",
...
def _compact_main_table(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "# Phase 10 External RL Baseline Results",
        "",
...
def _improvement_table(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "# Phase 10 Proposed Method Improvement over External Baselines",
        "",
...
def _plot_metric(
    ...
    *,
    ...
    axis.set_title(f"Phase 10 {ylabel}")
...
def write_phase10_artifacts(
    ...
    *,
    ...
    statistics = aggregate_phase10(summary_rows)
    effects = paired_phase10_effects(summary_rows)
    ...
    (tables / "external_rl_main_results.md").write_text(
        _compact_main_table(statistics),
        encoding="utf-8",
    )
    (tables / "phase8_improvement_over_external_rl.md").write_text(
